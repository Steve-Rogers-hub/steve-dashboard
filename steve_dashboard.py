...
def compute_tii_scores(df, macro_regime="Neutral"):
    ...
    # Final TII score
    # -------------------------
    # Base logic: quality + growth + momentum dominate, but balance sheet and value matter.
    x["tii_score"] = (
    ...
    penalty += np.where(x["debt_to_equity"] > x["debt_to_equity"].quantile(0.85), 4.0, 0.0)
    penalty += np.where(x["net_margin"] < x["net_margin"].quantile(0.15), 3.0, 0.0)
    penalty += np.where(x["revenue_growth"] < x["revenue_growth"].quantile(0.10), 3.0, 0.0)
