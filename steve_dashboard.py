...
def get_gspread_client():
    ...
    if client is None:
        ...
        st.stop()
    # Caption moved outside cached context — rendered by callers instead
    return client
...
def classify_conviction(row):
    ...
    When data_confidence is low the score is momentum-only, so we
    append '(Momentum)' to be transparent about the signal quality.
    """
    ...
    # Flag when score is based on momentum only (< 35% data completeness)
    try:
...
# ── Data mode banner ──────────────────────────────────────────────────────────
# Warn clearly when the full factor engine has nothing to work with.
_avg_confidence = pd.to_numeric(
    top_100_df.get("data_confidence"), errors="coerce"
).mean()
_fallback_pct = (
    (top_100_df.get("score_source", pd.Series()) == "Price fallback").mean() * 100
    if "score_source" in top_100_df.columns else 100.0
)

if pd.isna(_avg_confidence) or _avg_confidence < 35 or _fallback_pct > 80:
    st.warning(
        "⚠️ **Momentum-only mode** — The weekly scan CSV contains ticker symbols but no "
        "fundamental data (PE, margins, ROIC etc.). All conviction ratings are based on "
        "price momentum only. To activate the full factor engine, enrich the CSV with "
        "fundamental columns. See System Notes for the expected schema.",
        icon=None,
    )
elif _avg_confidence < 70:
    st.info(
        f"ℹ️ Average data completeness is {_avg_confidence:.0f}%. "
        "Some factor scores are estimated from sector medians. "
        "Enriching the CSV with more fundamental data will improve accuracy."
    )
...
with st.expander("System Notes"):
    st.markdown(
        ...
        ---
        **Conviction label guide**
        - Labels marked **(Momentum)** mean the score is based on price momentum only —
          no fundamental data was available for that stock.
        - To activate full factor scoring, enrich `weekly_scan_output.csv` with these columns:

        | Column | Description |
        |---|---|
        | `pe` | Trailing or forward P/E ratio |
